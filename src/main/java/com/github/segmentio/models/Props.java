...
public class Props extends HashMap<String, Object>
{
	...
	public Props(Object... kvs)
	{
		...
		if (kvs != null)
		{
			if (kvs.length % 2 != 0)
			{
				...
				logger.warning("Segment properties must be initialized with an "
	...
	public Props put(String key, Object value)
	{
		...
		{
			logger.warning(String.format("Key %s value %s not allowed because it is "
				+ "not of type String, Integer, Double, Boolean, or Date.", key, value));
		}
	...
	{
		if (isPrimitive(value) || value instanceof String || value instanceof Date || value instanceof DateTime
			|| value instanceof Props || value instanceof BigDecimal ||
			// http://stackoverflow.com/questions/2651632/how-to-check-if-an-object-is-a-collection-type-in-java
			value instanceof Collection || value instanceof Map || value instanceof Object[])
		{
			return true;
		}
		else
